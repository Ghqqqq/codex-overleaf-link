...
test('release version metadata is aligned for v2.1.3 while the RC handshake uses protocol 2', async () => {
...
test('buildBridgePingParams returns protocol 2 metadata with protocol 1 compatibility', () => {
  assert.equal(compatibility.BUILD_TARGET_VERSION, '2.1.3');
  assert.equal(compatibility.EXTENSION_PROTOCOL_VERSION, 2);
  assert.deepEqual(compatibility.buildBridgePingParams({ version: '2.1.3' }), {
...
test('classifyNativeCompatibility returns compatible for protocol 2 hosts with all required capabilities', () => {
  const result = compatibility.evaluateNativeCompatibility(nativeResponse(), { version: '2.1.3' });

  assert.equal(compatibility.classifyNativeCompatibility(nativeResponse(), '2.1.3'), 'compatible');
  assert.equal(result.status, 'ok');
  assert.equal(result.classification, 'compatible');
  assert.equal(result.requiredVersion, '1.0.0');
  assert.equal(result.recommendedVersion, '2.1.3');
  assert.equal(result.updateAvailable, false);
  assert.equal(result.updateCommand, canonicalInstallCommand('darwin'));
  assert.equal(result.releaseUrl, 'https://github.com/Ghqqqq/codex-overleaf-link/releases/tag/v2.1.3');
});
...
test('classifyNativeCompatibility keeps v1.0 protocol 1 hosts with required capabilities operational under v2.1', () => {
  ...
  const result = compatibility.evaluateNativeCompatibility(response, { version: '2.1.3' });

  assert.equal(compatibility.classifyNativeCompatibility(response, '2.1.3'), 'compatible');
  assert.equal(result.status, 'ok');
  assert.equal(result.classification, 'compatible');
  ...
  assert.equal(result.updateCommand, compatibility.buildInstallCommand('2.1.3', 'darwin'));
